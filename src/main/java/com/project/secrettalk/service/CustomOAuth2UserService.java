...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private Member saveOrUpdate(OAuthAttributes attributes){
    	Member member = null;
    	member = userRepository.findByEmail(attributes.getEmail()).map(entity -> entity.update(attributes.getName(),attributes.getEmail())).orElse(attributes.toEntity());
        System.out.println(member.getAge()+member.getLast_receive_date()+member.getCreateDate());
